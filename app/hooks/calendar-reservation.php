<?php
	...
	/* check access */
	$mi = getMemberInfo();
	if(!in_array($mi['group'], array('anonymous', 'Admins'))) {
	...
	<script>
		$j(function() {
			...
			Cal._fullCal = new FullCalendar.Calendar($j('#' + calId).get(0), {
				...
				eventSources: [
					{
						url: 'calendar-events-doctor-reservation.json.php',
						failure: function() {
			...
			Cal.Translate.ready(function() {
				Cal.newEventButtons.create([
					{
						type: 'doctor-reservation',
						color: 'default',
						title: AppGini.Calendar.Translate.word('new_x', { event: 'doctor reservation' }),
						table: 'events'
					},
				]);
			});
	...
	<div id="reservation-events-loading-error" class="hidden alert alert-warning text-center">
