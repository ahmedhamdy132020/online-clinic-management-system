...
class SummaryReport {
	...
	private function sql_fields($table) {
		...
		foreach($sql_fields_as as $prev_cap_next_field) {
			...
			if(!$sfai) {
				...
				$sql_fields[$sfai - 1]['caption'] = substr($prev_cap_next_field, 0, $first_comma);
				$sql_fields[$sfai] = ['field' => substr($prev_cap_next_field, $first_comma + 2)];
				$sfai++;
			}
		}
	...
	private function report_data() {
		...
		if(isset($this->date_field)) {
			$comparison_periods[] = array(
				date('Y-m-d', $this->ts_p0_start),
				date('Y-m-d', $this->ts_p0_end)
			);
			
			if(isset($this->ts_p1_start) && isset($this->ts_p1_end)){
				$comparison_periods[] = array(
					date('Y-m-d', $this->ts_p1_start),
					date('Y-m-d', $this->ts_p1_end)
				);
			}
			
			if(isset($this->ts_p2_start) && isset($this->ts_p2_end)){
				$comparison_periods[] = array(
					date('Y-m-d', $this->ts_p2_start),
					date('Y-m-d', $this->ts_p2_end)
				);
			}
		}else{
		...
		/* variables needed for parent table SQL */
		if(isset($this->parent_table) || isset($this->look_up_table)) {
			/*
				3 possibilities:
				1. field in same report table and is a lookup field ($this->look_up_table)
				2. field in a parent table and is not a lookup field ($this->parent_table)
				3. field in a parent table and is a lookup field ($this->parent_table)
			*/

			/* true for skipping permissions */
			$join_statment = get_sql_from($this->table, true);
			if(stripos($join_statment, ' where ') === false) $join_statment .= ' WHERE 1=1 ';

			if($this->look_up_table) {
				$sql_fields = $this->sql_fields($this->table);
				$label_field = $sql_fields[$this->label_field_index - 1]['field'];
			} elseif ($this->parent_table) {
				// here we have a parent_table (group field is from a parent table)
				// get parent table alias from join statement
				$m = array();
				preg_match('/`?' . $this->parent_table . '`?\s+as\s+\'?(.*?)\'?\s+on\s+/i', $join_statment, $m);
				$label_field = "`{$m[1]}`.`{$this->label}`";
			}
		}
		...
		foreach($comparison_periods as $period) {
			...
			if($this->parent_table || $this->look_up_table) {
				$query = "
						SELECT 
 		...
 		// handle case where label is from another table and is a lookup
		// by retrieving label captions given PKs
 		if($this->parent_label_is_lookup) {
 			// first, get list of PKs
 			$pks = array_map(function($pk) { return "'" . makeSafe($pk) . "'"; }, array_keys($items));
 			$pks_list = implode(',', $pks);

 			// next, build query parts for a query to retrieve parent captions along with stored lookup IDs
			$join_statment = get_sql_from($this->parent_table, true);
			if(stripos($join_statment, ' where ') === false) $join_statment .= ' WHERE 1=1 ';

 			$sql_fields = $this->sql_fields($this->parent_table);
 			$label_field = $sql_fields[$this->label_field_index - 1]['field'];

 			$query = "
 				SELECT
 					`{$this->parent_table}`.`{$this->label}` AS 'id',
 					{$label_field} AS 'label'
 				FROM
 					{$join_statment}
 				AND
 					`{$this->parent_table}`.`{$this->label}` IN ({$pks_list})
 			";

 			$res = sql($query, $eo);
			while($row = db_fetch_assoc($res)){
				if(!isset($items[$row['id']])) continue;
		...
		foreach($items as $i => $item) {
			if(empty($item['label'])) $items[$i]['label'] = NOLABEL;
		}

		$this->populate_missing_period_values($items);
	...
	private function populate_missing_period_values(&$items) {
		foreach($items as $i => $item) {
			$v1set = isset($items[$i]['value1']);
			$v2set = isset($items[$i]['value2']);
			$v3set = isset($items[$i]['value3']);
			
			if($v1set && !$v2set) $items[$i]['value2'] = 0.0;
			if($v1set && !$v3set) $items[$i]['value3'] = 0.0;
			if($v2set && !$v1set) $items[$i]['value1'] = 0.0;
			if($v2set && !$v3set) $items[$i]['value3'] = 0.0;
			if($v3set && !$v1set) $items[$i]['value1'] = 0.0;
			if($v3set && !$v2set) $items[$i]['value3'] = 0.0;
		}
	}
	...
	private function chart_data() {
		...
		$labels = array_slice(
			array_map(function($i) { return $i['label'] == NOLABEL ? 'None' : $i['label']; }, $items), 
			0, 
