$j(function() {
	...
	var buildReportForm = function() {
		...
		reportSections.add({
			...
			label: 'Data table<br><img src="images/summary-reports-data-table-section.png" class="img-responsive">',
			type: 'checkbox',
			name: 'data_table_section',
			init: true
		}).add({
			id: 'bar-chart-section',
			label: 'Bar chart<br><img src="images/summary-reports-bar-chart-section.png" class="img-responsive">',
			type: 'checkbox',
			name: 'barchart_section'
		}).add({
			id: 'pie-chart-section',
			label: 'Pie chart<br><img src="images/summary-reports-pie-chart-section.png" class="img-responsive">',
			type: 'checkbox',
			...
			help: '<i>This is an experimental feature and not yet supported by browsers.</i>'
		})
		...
		.add({
			...
			id: 'previous-reports',
			init: function() {
				if(AppGiniPlugin.table_name == undefined) return '';
				if(!AppGiniPlugin.table_name.length) return '';

				return JSON.stringify(getReports(AppGiniPlugin.table_name));
			} 
		})
		...
		reportData.add({
			...
			'<div id="group-table-no-ancestors" style="margin: 0 10%;" class="hidden">' +
			...
			'<div class="hidden" id="summarized-value-validation" style="margin: 0 10%">' + 
				'This table has no fields to summarize. You can only use count.' +
			'</div>'
		).addSeparator().add({
			id: 'date-field',
			type: 'select',
			label: 'Date field used to filter the report',
			labelClasses: modalLabelClasses,
			controlWrapperClasses: modalInputClasses,
	...
	AppGiniPlugin.reportsList = AppGiniPlugin.itemsList({
		...
		itemTitle: function(item) { return item.title; },
